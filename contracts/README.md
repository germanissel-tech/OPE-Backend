...
| `api-map.yaml`         | Mapa del contrato: toda operación construida o planeada, con consumidor, esquema de seguridad, capacidades, feature de `specs/` (construida) o hito del `roadmap` (planeada) y fuente; ciclo de vida `planned → built → deprecated → retired`. | fuente                    | `check:api-map`, `contract:docs` (superficie planeada), la réplica de capacidades en `src/` | `check:api-map` (mapa ↔ contrato en los dos sentidos)                                         |
