...
| `messages.json`           | El corpus curado del release (feature 027): los textos que una persona ve, por familia de mensaje, idioma y voz, cada uno con su versión. Los escribe OPE y se reusan entre merchants; lo que el merchant elige es qué versión, con qué voz y en qué idiomas (constitución X). No es una política: es contenido, y la revisión previa a servirse es la revisión del cambio (constitución VIII). | fuente            | `composition/corpus-config.ts` → `readCorpus` → `CuratedText.of` (dominio); el módulo `messages` lo sirve desde memoria en el camino crítico                                 | `CuratedText.of` rechaza un texto vacío, más largo que el contrato o con un hueco sin resolver; un corpus incoherente no arranca el servidor | `OPE_MESSAGE_CORPUS` (ruta de otro archivo)               | Al arrancar, siempre                                                                                            |
...
| `messages.json`           | —                                                      | sin esquema propio: su forma la juzga `readCorpus` y sus reglas `CuratedText.of`                                      |
