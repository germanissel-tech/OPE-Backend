...
| 12  | Ajustes internos del repo                         | —                                                                                             | Constitución cita `talle_calce`, `precio_valor`, `cambios_devoluciones`; ADR-010 remite D3–D6 a `04`     | PATCH de la constitución (`fit`, `price`, `returns`) + gate `check:identifiers`; ADR nuevo con las decisiones de producto confirmadas; ADR-010 corregido |
...
- ADR-025 revisado (push es un modo, no el caso base); un ADR nuevo, "Decisiones de producto confirmadas" (D-B, D-C, D-E, D-F, D-G); ADR-010 corregido (D3–D6 → `01 §13`).
