...
`specs/014-auditoria-integral/` · **Estado global**: `rejected` (§7)
...
**Sesiones**: tres, todas el 2026-09-19 (fase 0 · fases 1 parcial · fases 1 a 5), registradas en `trabajo/avance.md` con la tarea siguiente al cierre de cada una; los commits de cierre por fase están en `git log` de la rama.

**Qué se corrió además en las fases 1–5**: `run-gates.mjs` sobre 15 alcances (§2.2); la mutación completa (57 min, 99,15 %, `trabajo/gates/mutation-full-summary.json`); la suite con reporter JSON para medir tiempos por archivo (§3.E); `tests/integration/{catalog-size,ingest-latency}.test.ts` con salida visible para las cifras de §3.B; un preflight CORS con `startTestApp` para F-051; `npm outdated` para A-249; `npm run quality` cronometrado (26 s). `verify-finding.mjs` sobre cada `trabajo/hallazgos/fase-N.json` y sobre el conjunto al cierre.

**Qué no se pudo verificar**: (1) el comportamiento bajo tráfico concurrente real — las mediciones de latencia usan `inject` (una request por vez), y la interferencia entre planos (F-045) se dedujo del modelo de un solo event loop más el tiempo síncrono medido, no de una corrida concurrente; (2) la ejecución del workflow de CI (no hay push en esta rama; `ci.yml` se leyó y su prueba `tests/hooks/ci.test.ts` pasa); (3) el estado abierto de stryker-js#6210 (sin red); (4) las cifras históricas de los quickstarts (se citan como históricas). Los tres documentos del MVP fueron legibles en todas las sesiones.
...
Hallazgos de las fases 2–4 por alcance, fuera de los ejes de lectura (dimensiones B–F): `ingestion` F-043 (puertos de lectura; alcanza a experiment, catalog, decision, ledger), `catalog` F-044, `infrastructure` **F-045**, **F-051**, F-052, F-057, `decision` F-046, `ledger` F-047, `merchant` F-048, F-053, F-058, `barrier` F-052 (2ª), `outcomes` **F-062**, transversales F-055, F-056 (`tests/`), F-054 (`scripts/`), **F-063** (constitución). Cuadro final: 15 filas × 7 ejes arriba, con estas adiciones por dimensión.

Refutados en la fase 1: F-002 (`src/application/shared-kernel/decorators/logged-use-case.ts:19`), F-010 (`src/interface-adapters/http/boundary.ts:12`), F-042 (`src/application/decision/services/decision.service.ts:134`) — motivos en §5.
...
Marcadores (T246): decisiones tomadas en código que convendría marcar — la excepción de `incentive` frente a VII (F-062), el caso base push frente a X (F-063), el hook sin `arch` (F-064, refutado: ADR-011) y los porcentajes en el dominio (F-031) son `DECIDIDO` de hecho sin texto que lo diga; ninguna requiere un `ABIERTO`.
...
| A-243 | `FR-063`  | probado              | Cada hallazgo de diseño MUST pasar una segunda revisión que intenta refutarlo; sólo los c… | ✓ `references/refutacion.md`; esta auditoría refutó 9 hallazgos (§5)                                                                                                                                                                |
...
| A-417 | `FR-020`  | probado por cabecera | El gate MUST ser una función pura: candidato + evidencia + perfil del merchant ⇒ `acceptable` o `unacceptable`… | ✓ cabecera: `unit/domain/selection/quality-gate.test.ts`, `unit/no-op-reasons.test.ts`                                                                                                                                         |
...
9 hallazgos propuestos no sobrevivieron a la refutación (`references/refutacion.md`); están en `trabajo/hallazgos/refutados.json` con la fase en que nacieron y siguen en su `fase-N.json` con `status: "refuted"`. La segunda pasada de la fase 5 sobre los 56 confirmados retiró uno (F-064).

- **F-002** · `src/application/shared-kernel/decorators/logged-use-case.ts:19` · nació en la fase 1 (fuente propuesta `clarity:log-outcome-misleading`) — By type a use case returns Result<T, E extends DomainError> (ADR-023), so a false `ok` without a DomainError cannot come from a compliant use case; the test documents the chosen behaviour for non-Result responses. Not observable in this codebase.
- **F-010** · `src/interface-adapters/http/boundary.ts:12` · nació en la fase 1 (fuente propuesta `clarity:misplaced-helper`) — CLAUDE.md § Notas operativas (Outcomes) decides it: what controllers share at the boundary (`instantOf`, `linesOf`, `idempotent`) lives in `http/boundary.ts` because a file under `controllers/` is one operation (shape rule). A documented placement, not a defect.
- **F-042** · `src/application/decision/services/decision.service.ts:134` · nació en la fase 1 (fuente propuesta `ADR-027`) — ADR-027 forbids the orchestrator to infer or to choose; skipping the gate when the decision policy already said the evidence cannot sustain the barrier is neither: the commercial policy refuses on `evidenceReason` regardless (src/domain/commercial/commercial-policy.ts:198), so the verdict is identical with or without the gate, and what changes is only whether the ledger records candidates judged against evidence the policy already declared missing. The condition is a short-circuit on the policy's own output, not a rule of its own. S-05 stays on the file's size and Stryker disables (fase 2, T197), not on this line
- **F-049** · `src/application/decision/services/decision.service.ts:230` · nació en la fase 2 (fuente propuesta `ADR-027`) — `evidenceOf` translates `ProductTruth` — a type of the catalog _application_ module — into the input types of three authorities and the ledger's record; `selectionOf` translates the gate's and the commercial policy's outputs into the ledger's `DecisionSelection` (strings only, by ADR design). A domain module cannot import an application type (rings, ADR-013), so the translation can only live in the application, and ADR-027 gives the orchestrator exactly this job ("builds the context"). The file is at 259 of 300 lines with four `Stryker disable` (the handoff counts five), each justified in place; S-05 stays as a size risk for the 014/015 additions (§6), not as a defect. The full mutation run leaves one survivor here (`decision.service.ts:137: Survived: false`, an optional-key spread) that fase 3 lists
- **F-050** · `src/domain/ledger/decision.ts:125` · nació en la fase 2 (fuente propuesta `ADR-023`) — The six `throw new Error` in `src/domain/**` are the ones CLAUDE.md § Cómo se escribe un caso de uso reserves for programming errors (→ 500): a default policy that does not build (load time), a batch without events after the contract guaranteed one, and a ledger record that fits no shape. None is a business condition a client can cause; `src/application/**` has none. One of them is worth a line in §6: `DecisionBase.rehydrate` throwing on a corrupt row means that, with the 017 store, one bad row makes `NotifyOrderUseCase` (via `bySession`) and `ConfirmExposureUseCase` (via `find`) answer 500 for that session or decision — a data-quality alarm, not a fail-closed path
- **F-059** · `contracts/paths/events.yaml:12` · nació en la fase 3 (fuente propuesta `ADR-007`) — Schemathesis' "schema validation mismatch" on five operations is not a contract stricter than the server: three operations (`POST /v1/orders`, `POST /v1/returns`, `PUT /v1/catalog`) answer 401 to every generated request because the runner sends only the ingest key (F-054), and the other two reject most generated bodies through invariants the schema cannot express and ADR-007 places in `x-invariants` on purpose (session/visitor mismatch, instants outside the tolerance, unknown decision). 2433/2433 cases pass; the warning is informative and its cause is now written here
- **F-060** · `tests/integration/ingest-latency.test.ts:22` · nació en la fase 3 (fuente propuesta `constitution#IV. Dos caminos, dos garantías`) — The latency tests are gates already: ingest-latency and outcomes-latency assert `p95 < 50 ms` (a third of the 150 ms design objective) and catalog-size asserts `< 2 000 ms` for the pilot size; only `npm run test:load` never fails by its figures, and constitution IV says the objective "no es SLA hasta medirlo bajo tráfico real", so an informative load run is what the constitution asks for. Measured today: ingest p50 0.67 ms, p95 1.28 ms; catalogue 140 ms
- **F-061** · `tests/helpers/test-app.ts:1` · nació en la fase 3 (fuente propuesta `constitution#III. La medición precede`) — Merchant A of the helper is `treatmentPercent: 100`, so most integration tests never see CONTROL; but tests/integration/assignment.test.ts configures its own 50/50 experiment, picks the first CONTROL and TREATMENT visitors with the domain function (`visitorIn`) and proves end to end that CONTROL runs the whole pipeline, resolves `NO_OP` `control-arm`, records arm and experiment in the ledger and never leaks them in a response; tests/unit/application/decision/decision.service.test.ts covers CONTROL through the same inference and the selection it would have chosen. Not ad hoc: the 50/50 fixture is the point of that file
- **F-064** · `lefthook.yml:1` · nació en la fase 4 (fuente propuesta `spec:002#FR-041`) — ADR-011 (feature 003, posterior a la 002) decide por escrito el contenido del hook: "`pre-commit` corre formato y lint sobre lo staged y `typecheck`. No corre `contract:check` ni pruebas: eso es de CI"; `git log lefthook.yml` muestra que `arch` nunca estuvo en el hook (711f15a) y CLAUDE.md § Flujo lo repite. Una decisión escrita posterior sustituye la redacción de la 002 FR-041, que no fue enmendada: queda como nota de prosa (la spec 002 y su quickstart siguen diciendo "previo a commit"), no como defecto. `npm run arch` corre en `quality` y en CI

Sin duplicados entre fases: los pares que comparten raíz se citan mutuamente en vez de repetirse (F-045/F-057 event loop y `bodyLimit`; F-017/F-029 números de feature; F-021/F-030/F-033 conocimiento repetido bajo el umbral de los gates; F-042/F-049 sobre `DecisionService`; F-012/F-045 sobre `build-server.ts`).
...
| Renumeración del mapa: la 014 pasa a ser esta auditoría; cuatro comentarios de `src/` (F-029), uno de `modules/experiment.ts` (F-017) y `contracts/components/schemas/Incentive.yaml:7` (publicado) citan features futuras por número | `freshness.ts:3`, `policy-directory.ts:3`, `intervention.ts:19`, `candidate.ts:5`, `Incentive.yaml:7`                                                          | la próxima feature del mapa             |
...
Regla fija (`.claude/skills/auditing-architecture/SKILL.md` § Paso 7; spec 014 FR-018): `rejected` si algún gate bloqueante está en rojo o hay algún hallazgo `high`; `changes-required` si no hay `high` y hay `medium`; `approved` si sólo hay `low` o nada. Sin puntuaciones.

- Gates bloqueantes en rojo: **0** (§2.1: los siete comandos globales en exit 0; §2.2: 15 alcances sin gate rojo sobre código del producto — el único `fail` es un artefacto de la herramienta sobre `generated/api.d.ts`).
- Hallazgos confirmados y verificados: **55** — high **6**, medium **17**, low **32**; refutados 9.

| Fase | Confirmados | high | medium | low | Refutados |
| ---- | ----------- | ---- | ------ | --- | --------- |
| 1    | 39          | 1    | 15     | 23  | 3         |
| 2    | 6           | 2    | 1      | 3   | 2         |
| 3    | 8           | 1    | 1      | 6   | 3         |
...
⇒ **Estado global: `rejected`**, por los 6 hallazgos de severidad alta: F-007 (`ADR-024`), F-043 (`constitution#II. Fail-closed`), F-045 (`constitution#IV. Dos caminos, dos garantías`), F-051 (`ADR-025`), F-062 (`constitution#VII. OPE observa comportamiento, no personas`), F-063 (`constitution#X. Puertos en los dos bordes`).
