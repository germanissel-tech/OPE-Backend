...
| `audited-operations.js` | `AUDITED_OPERATIONS`: las operaciones que el contrato manda auditar, derivadas del bundle y del mapa (consumidor `admin` más una capacidad que no es de lectura). | derivado | el grafo de composición, por `interface-adapters/http/typed.ts` | `contract:types:check` |
