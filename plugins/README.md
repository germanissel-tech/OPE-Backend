...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `auditable-architecture/` | Auditoría de arquitectura por método (`auditing-architecture`, guiada por `audit.profile.json`) y, con D-02, el acondicionamiento de un repositorio. Su README explica la instalación. | fuente            | Claude Code (plugin habilitado); `tests/audit/` lo ejecuta por ruta | `tests/audit/audit.test.ts`, `tests/audit/plugin-isolation.test.ts`, `claude plugin validate` |
