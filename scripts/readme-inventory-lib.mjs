...
/**
 * The `x-*` keys a set of YAML sources declares (the contract, not its bundle), for the
 * extensions table of `contracts/README.md`.
 * @param {readonly string[]} contents file contents
 * @returns {string[]} sorted, unique
 */
export function extensionKeys(contents) {
  const keys = new Set();
  for (const content of contents) {
    for (const match of content.matchAll(/^\s*(x-[a-z0-9-]+):/gmu)) keys.add(match[1] ?? "");
  }
  return [...keys].filter((k) => k !== "").sort();
}

/**
 * The extensions the table names, in its first column.
 * @param {string} markdown
 * @returns {string[]} sorted, unique
 */
export function documentedExtensions(markdown) {
  const lines = sectionOf(markdown, "Extensiones");
  const table = lines === null ? null : parseTable(lines);
  return table === null ? [] : [...new Set(table.rows.map((row) => row.entry))].sort();
}
