// Gate adapter `duplication` (findings-v1, ADR-032): scripts/check-duplication.mjs --json, repository-wide, with its
...
const RULES = ["duplication"];

const call = invocation(process.argv.slice(2));
if (call.listRules) {
  emitRules(RULES);
} else {
  const r = capture(process.execPath, [path.join(repoRoot, "scripts", "check-duplication.mjs"), "--json"]);
  /** @type {{ findings: { file: string; line?: number; rule: string; message: string; mode?: string }[]; error?: string }} */
  let parsed;
  try {
    parsed = JSON.parse(r.stdout);
  } catch {
    fail(`check-duplication.mjs did not answer JSON: ${(r.stderr || r.stdout).split(/\r?\n/u)[0] ?? ""}`);
  }
