...
const actual = existsSync(generatedTypesPath) ? readFileSync(generatedTypesPath, "utf8").replace(/\r\n/g, "\n") : "";
if (actual !== expected) {
  console.error(`Tipos generados desactualizados: ${generatedTypesPath} no coincide con el contrato. Corré npm run contract:types`);
  process.exit(1);
}
