...
- 2026-09-22 CI del push (#28): `mutation` en verde, cero sobrevivientes; `checks` falló en la prueba del doctor sobre este repo porque la fuente `mvp:` apunta fuera del repositorio (`../01-*.md`, `dirEnv`) y en el runner no existe: el doctor la reporta `missing`, que es la verdad en CI. La prueba exige `ready` sólo a las fuentes del repositorio y tolera las externas.
- 2026-09-22 (decisión del dueño): **sin plugin**. Las dos skills pasan de `plugins/auditable-architecture/skills/` a `.claude/skills/` como las de spec-kit; desaparecen `plugins/`, `.claude-plugin/marketplace.json` y `.claude/settings.json`, y el marketplace `ope` registrado en `~/.claude/plugins/known_marketplaces.json` se quitó (`claude plugin marketplace remove ope`). Motivo: el mecanismo de plugins registra marketplace y plugin fuera del repositorio (instalación global). `tests/audit/plugin-isolation.test.ts` pasa a `skills-isolation.test.ts` (mismas garantías sobre los dos directorios); `check:language` vuelve a recorrer `.claude/skills/*/scripts`; `check:identifiers` suma `.claude/skills`; knip, ESLint y `tsconfig.typecheck.json` apuntan a `.claude/skills/*/evals/*/fixture`; `audit.profile.json` y `write-profile` referencian el esquema en `.claude/skills/auditing-architecture/scripts/`. `claude plugin validate` ya no aplica. ADR-032 enmendado.
