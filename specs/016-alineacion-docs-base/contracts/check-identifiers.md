...
```
node scripts/check-identifiers.mjs [--docs <dir,dir>] [--constitution <file>] [--bundle <file>] [--src <dir>] [--allowlist <file>]
```
...
- Existe: token (`\b…\b`) en `contracts/dist/openapi.yaml`, `contracts/problem-types.yaml`, `contracts/no-op-reasons.yaml`, `contracts/api-map.yaml` o cualquier `src/**/*.ts`.
