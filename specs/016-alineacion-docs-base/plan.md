...
renumerado y reordenado con el alcance de cada feature; glosario), contrato con la cadena (cadena de
...
**Constraints**: un cambio incompatible del contrato (aceptado por la marca `building`,
1.3.0, `/v1/`; R-04) y uno compatible (`locale`); cero excepciones nuevas de lint, idioma o mutación; commits por
...
| VI. Identidad explícita / contrato primero     | **Sí**   | `status`/`correlation` y `locale` entran por `contracts/` antes que el código; el cambio incompatible entra con la marca `building` (ADR-003, precisión 2026-09-20; R-04); `contract:diff` lo reporta y lo acepta sólo por la marca |
...
| Mapa del contrato / superficie HTTP            | **Sí**   | Sin operaciones nuevas; rutas `/v1/`; `features:` renumeradas y reordenadas (R-03); `check:api-map` verde                                                                                                                           |
...
├── contracts/
│   ├── contract-evidence-chain.md   # esquemas, versión, marca building, glosario, código
...
contracts/
├── openapi.yaml                                 # info.version 1.3.0, x-stability: building
├── paths/*.yaml                                 # ejemplos con status + correlation
├── components/schemas/{OrderStatus,Correlation,OrderResult,ReturnResult,PageContext}.yaml
└── api-map.yaml                                 # features 016–022 (R-03)
src/
...
CLAUDE.md                                        # fila del gate; regla de la marca building
tests/
├── governance/identifiers.test.ts + fixtures/identifiers/{ok,unknown,allowlist-without-reason}/
├── integration/{orders,order-corroborations,returns,ingest-events,...}.test.ts   # status + correlation, locale
...
Sin violaciones que justificar. El cambio incompatible sin versión mayor entra por la
precisión de ADR-003 (marca `building` mientras no haya consumidores), registrada en la
constitución 1.4.2; no es una excepción silenciosa: `contract:diff` la reporta y
`release-check` la avisa.
