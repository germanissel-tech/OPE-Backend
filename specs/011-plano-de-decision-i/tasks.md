...
- [ ] T002 [P] Kernel: crear `src/domain/shared-kernel/intervention.ts` con `ANCHORS`, `Anchor`,
...
- [ ] T003 [P] `src/domain/ingestion/event.ts`: listas `EVENT_TYPES`, `BLOCKS`,
...
- [ ] T004 [P] `src/domain/ingestion/event-batch.ts`: `focus(): { productId?: string;
...
- [ ] T005 [P] Ledger: `src/domain/ledger/decision.ts` gana `DecisionInference` y
...
- [ ] T006 [P] `src/application/catalog/services/product-truth.service.ts`: `ProductTruth`
...
- [ ] T007 `npm run quality && npm test` en verde (`knip`: `DecisionRecorder` y `product` usados por
...
- [ ] T008 [P] [US3] Dominio `src/domain/barrier/`: `signals.ts` (`EventKey`, `EventRef`,
...
- [ ] T009 [P] [US3] Dominio `src/domain/decision/`: `errors.ts` (`InvalidPolicyVersion`,
...
- [ ] T010 [US3] `src/domain/decision/default-policy.ts`: `DEFAULT_DECISION_POLICY` construida
...
- [ ] T011 [US3] `src/composition/config.ts`: `MerchantConfig.decisionPolicy?: DecisionPolicy`;
...
- [ ] T012 [US3] Puerto y gateway: `src/application/decision/ports/decision-policy-directory.ts`
...
- [ ] T013 [US1] `src/domain/decision/decision-policy.ts`: `VerdictInput`, `Verdict`,
...
- [ ] T014 [P] [US1] `src/domain/decision/session-state.ts`: `class SessionState { static
...
- [ ] T015 [P] [US1] Aplicación `barrier`: `src/application/barrier/ports/barrier-inference.ts`
...
- [ ] T016 [P] [US1] Puerto y gateway de sesión: `src/application/decision/ports/session-state-store.ts`
...
- [ ] T017 [US1] Puerto de la ingesta: `src/application/ingestion/ports/decision-plane.ts`
...
- [ ] T018 [US1] Orquestador `src/application/decision/services/decision.service.ts`:
...
- [ ] T019 [US1] Ingesta: `src/application/ingestion/use-cases/ingest-batch.use-case.ts` con
...
- [ ] T020 [US1] Composición: `src/composition/modules/barrier.ts` (`BarrierPorts { inference:
...
- [ ] T021 [US1] `tests/integration/decision-plane.test.ts` (merchant con `treatmentPercent:
...
- [ ] T022 [US1] `tests/integration/isolation.test.ts`: A con umbral 0,9 y B con 0,4 → la
...
- [ ] T023 [US1] Contrato y glosario: `contracts/no-op-reasons.yaml` (+ 6, de
...
- [ ] T024 [US1] `npm run format:check && npm run quality && npm run typecheck && npm test &&
...
- [ ] T025 [US2] `tests/integration/decision-evidence.test.ts` con reloj controlado
...
- [ ] T026 [US2] Si T025 revela un orden distinto en `verdict`, corregir en
...
- [ ] T027 [US4] `tests/integration/decision-plane.test.ts`: ingesta → `INTERVENE` →
...
- [ ] T028 [US4] `config/dev-merchants.json`: `treatmentPercent: 100` en el experimento de
...
- [ ] T029 [P] `docs/adr/026-politica-de-decision-por-merchant.md` (estado `aceptada`, fuente
...
- [ ] T030 [P] `docs/adr/024-dominio-rico.md`: los dos PROPUESTO pasan a "Cerrado en 011"
...
- [ ] T031 [P] `CLAUDE.md`: módulos `barrier` y `decision` en la lista y en el mapa; nota
...
- [ ] T032 `npm run test:mutation` (matar supervivientes con pruebas dirigidas; `Stryker disable`
