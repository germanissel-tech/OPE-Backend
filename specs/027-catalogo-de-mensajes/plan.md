...
corpus es un activo del release; lo que se publica por merchant es sólo lo que varía —voz, cadena de
idiomas, correspondencia de etiquetas—.

El vestido del mensaje es **una autoridad propia**, después del veredicto comercial y antes del
ledger. No es trabajo del orquestador: el principio I se lo prohíbe por su nombre.
...
**Testing**: unitarias del dominio y de la autoridad, integración por `fastify.inject`, réplicas
...
**Performance Goals**: el vestido ocurre en el camino crítico de decisión, así que **sin I/O de red
...
**Scale/Scope**: un módulo nuevo (`messages`), una autoridad, tres puertos, un activo del release,
...
| #    | Principio                              | Veredicto                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| I    | Separación de autoridades              | **Es el principio que más manda acá.** Dice literalmente que el orquestador «MUST NOT … **elegir mensajes**», así que el vestido va en su propia autoridad, en su propio módulo, invocada en orden. **No emite veredicto**: la política comercial sigue siendo la única que decide intervenir; esta autoridad sólo informa si puede entregar, como ya hace `DecisionRecorder` con el ledger (research R-03)                                                                                                                                                        |
...
| V    | Aislamiento por merchant               | **Se respeta.** La voz, la cadena de idiomas y la correspondencia de etiquetas son configuración del merchant, resuelta por su credencial; el corpus compartido no lleva nada de ningún merchant. La feature incluye pruebas de aislamiento porque toca configuración                                                                                                                                                                                                                                                                                              |
...
| IX   | Nada entra al reporte sin trazabilidad | **Se respeta.** `DecisionRecord.intervention` ya lleva la versión del mensaje; lo que cambia es que deja de ser un placeholder. Por eso el vestido va **antes** del ledger: el registro tiene que poder decir qué leyó la persona                                                                                                                                                                                                                                                                                                                                  |
...
**Gate de superficie HTTP**: aplica. Cambia `Intervention` (lleva el texto), cambia `Locales`
(cadena en vez de tag único), se agregan campos a la configuración del merchant, se agrega el motivo
...
sustantivos nuevos —voz, valor de atributo, versión de mensaje, cadena de idiomas— y la fila del
...
├── components/schemas/Locales.yaml         # cadena de repliegue en vez de un tag
...
├── services/message.service.ts             # la autoridad
...
2. **El vestido es una autoridad posterior al veredicto** (R-03). El principio I prohíbe al
   orquestador elegir mensajes; el borde HTTP no puede hacerlo porque el ledger tiene que registrar
   la versión. Va entre la política comercial y el ledger, con la forma de `DecisionRecorder`.
...
6. **La cadena de idiomas reemplaza al salto único** (R-06), y el idioma manda sobre la voz.
...
1. **El principio I decide dónde va el vestido, por su nombre.** La spec describía el resultado; el
   principio dice literalmente que el orquestador no elige mensajes. Eso convierte una decisión de
   diseño en una obligación, y de paso resuelve que `decision.service.ts` —hoy en 267 de 300 líneas
   permitidas— no crezca.
2. **El principio X enumera el «catálogo de mensajes» como configuración por merchant.** Es una
   tensión real con la decisión de que los textos los escribe OPE, y está resuelta con un reparto,
   no ignorada. El dueño tiene que verla.
...
1. **US1 — el SDK muestra texto real.** El módulo, la autoridad, el corpus con su estructura
...
3. **US3 — la cadena de idiomas.** Independiente de US2.
4. **US4 — los valores sin mapear.** Última: el sistema es correcto sin ella, sólo ciego.
