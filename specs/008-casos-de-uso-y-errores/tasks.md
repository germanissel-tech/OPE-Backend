...
- [x] T002 [P] Crear `src/domain/shared-kernel/errors.ts`: `SafeDetails = Readonly<Record<string,
...
- [x] T003 [P] Crear `src/domain/shared-kernel/result.ts`: `Result<T, E extends DomainError> =
...
- [x] T004 [P] Crear `src/application/shared-kernel/use-case.ts` con `interface UseCase<Request,
Response> { execute(request: Request): Promise<Response> }`; exportar desde
      `src/application/shared-kernel/index.ts`
- [x] T005 [P] Crear `src/domain/ledger/errors.ts`: `LedgerUnavailable` (`ledger-unavailable`),
...
- [x] T006 [P] Crear `src/domain/ingestion/errors.ts`: `SessionVisitorMismatch`
...
- [x] T007 [P] Crear `src/domain/merchant/errors.ts`: `Unauthorized` (`unauthorized`),
...
- [x] T008 Cambiar `src/domain/ingestion/batch.ts`: `checkBatch` devuelve `Result<void,
...
- [x] T009 Cambiar los puertos de escritura a `Result` (data-model.md): `record()` de
...
- [x] T010 Adaptar los gateways a `ok()`/`fail(new LedgerUnavailable(...))`:
...
- [x] T011 Adaptar provisionalmente los consumidores actuales de `RecordOutcome` para que la
...
- [x] T012 [P] Crear `src/interface-adapters/http/to-problem.ts`: `toProblem(error: DomainError,
...
- [x] T013 [P] Pruebas de tipos en `tests/types/result.test-d.ts`: `Result<number, Error>` no
      compila (`@ts-expect-error`), `switch` sobre `error.code` de una unión sin un caso no
      compila, `error.module` estrecha a `ModuleName`
- [x] T014 [P] Pruebas unitarias `tests/unit/domain/shared-kernel/errors.test.ts` y
      `result.test.ts`: `name` es el nombre de la clase, `details` por defecto `{}`, `instanceof
DomainError`, `ok`/`fail` producen la forma exacta
- [x] T015 Commit `refactor(shared-kernel): raíz DomainError, Result, contrato UseCase y puertos
...
- [x] T016 [P] [US1] Crear `src/application/system/use-cases/get-service-health.use-case.ts`:
...
- [x] T017 [P] [US1] Crear `src/application/merchant/use-cases/resolve-ingest-key.use-case.ts`:
...
- [x] T018 [US1] Crear `src/application/ledger/use-cases/confirm-exposure.use-case.ts`:
...
UseCase<ConfirmExposureRequest, Result<ExposureRecordStatus, LedgerError>>`; eliminar
      ...
      `r.error.code`)
- [x] T019 [US1] Crear `src/application/ingestion/use-cases/ingest-batch.use-case.ts`:
...
- [x] T020 [US1] `npm run quality && npm test && npm run test:contract` en verde; verificar con
...
- [x] T021 [P] [US1] Regla ESLint tipada `scripts/lint/use-case-shape.mjs` (`ope/use-case-shape`,
...
- [x] T022 [P] [US1] Regla ESLint tipada `scripts/lint/dependencies-are-interfaces.mjs`
...
- [x] T023 [US1] Commit `refactor(application): casos de uso como clases UseCase con
...
- [x] T024 [US2] Crear `src/application/experiment/services/assignment.service.ts`: `interface
...
- [x] T025 [US2] Mover `tests/unit/application/experiment/assign-visitor.test.ts` a
...
- [x] T026 [P] [US2] Reglas en `.dependency-cruiser.cjs`: `use-cases-no-use-cases` (from
...
- [x] T027 [US2] Commit `refactor(experiment): AssignmentService como servicio de aplicación;
...
- [x] T028 [P] [US3] Regla ESLint tipada `scripts/lint/domain-error-shape.mjs`
...
- [x] T029 [P] [US3] Regla ESLint tipada `scripts/lint/no-throw-domain-error.mjs`
...
- [x] T030 [P] [US3] Regla ESLint `scripts/lint/no-generic-catch-in-application.mjs`
...
- [x] T031 [P] [US3] Prueba de réplica en `tests/unit/domain/error-codes.test.ts`: recorre
...
- [x] T032 [US3] Prueba de aplicación en `tests/unit/application/ingestion/ingest-batch.use-case.test.ts`:
...
- [x] T033 [US3] Commit `feat(lint): reglas de forma de errores de dominio y réplica de códigos
...
- [x] T034 [P] [US4] Pruebas `tests/unit/http/to-problem.test.ts`: para cada clase de
...
- [x] T035 [P] [US4] Regla `problem-translation-only-in-http` en `.dependency-cruiser.cjs`:
...
- [x] T036 [US4] Verificar que `src/interface-adapters/http/controllers/**` y
...
- [x] T037 [US4] Commit `refactor(http): toProblem como única traducción de errores de dominio
...
- [x] T038 [US5] Crear `src/application/shared-kernel/decorators/logged-use-case.ts`: `class
...
O>, deps: { clock: Clock; logger: Logger })`; `execute` mide con `clock.now()` y loguea `info`
      ...
      `src/application/shared-kernel/index.ts`
- [x] T039 [P] [US5] Pruebas `tests/unit/application/shared-kernel/logged-use-case.test.ts` con
...
- [x] T040 [US5] Aplicar en composición: `src/composition/modules/{system,ingestion,ledger}.ts`
...
- [x] T041 [US5] Commit `feat(application): decorador de registro operativo sobre el contrato
...
- [x] T042 [P] `docs/adr/023-casos-de-uso-servicios-y-errores.md`: `estado: aceptada`; ajustar
      el texto si la implementación cambió algún detalle (nombres de reglas, límite)
- [x] T043 [P] `CLAUDE.md`: sección "Cómo se escribe un caso de uso nuevo" (archivo en
...
- [x] T044 [P] (no aplica: el README sólo enlaza a `docs/adr/`) `README.md`: párrafo de la capa de aplicación (contrato, servicios, errores) si el
      README describe la arquitectura
- [x] T045 `npm run test:mutation` sobre el diff y matar los mutantes supervivientes con
      aserciones (no con excepciones)
- [x] T046 `specs/008-casos-de-uso-y-errores/quickstart.md`: completar "Estado al cierre" con
      fecha y salida de `npm test`, `npm run quality`, `npm run test:mutation`
- [x] T047 Commit `chore(008): ADR-023 aceptada, guía de agentes y cierre de la feature`; `npm
