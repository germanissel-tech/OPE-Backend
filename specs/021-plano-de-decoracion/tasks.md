...
- [ ] T001 Confirmar la base de la rama: `git log --oneline main..HEAD` incluye los 23 commits de la 020. Si la 020 ya está en `main`, rebasear `021-plano-de-decoracion` sobre `main` antes de
      tocar nada.
...
- [ ] T002 Crear `scripts/contract-audited-operations-lib.mjs`: deriva del bundle
...
- [ ] T003 Enganchar esa salida en `scripts/contract-types.mjs`, escribiendo
...
- [ ] T004 [P] Agregar la fila de `audited-operations.d.ts` al inventario de `generated/README.md`
      (ADR-032; `tests/docs/readmes.test.ts` falla si falta).
- [ ] T005 Correr `npm run contract:types` y verificar que el tipo nombra **exactamente diez**
...
- [ ] T006 Verificar que `npm run contract:types:check` **falla** con la salida desactualizada
      (borrar una línea del generado y correrlo), y vuelve a verde al regenerar.
- [ ] T007 [P] Reexportar el tipo desde `src/interface-adapters/http/typed.ts`, junto a
...
- [ ] T008 Prueba de la derivación en `tests/governance/audited-operations.test.ts` (proyecto
